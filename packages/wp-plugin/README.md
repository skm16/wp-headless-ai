...
**Integration test coverage (Phase 1.1, landed 2026-06-03):** the `acf_no_schema_skips` populated-ledger branch is now exercised end-to-end against a real wp-env + ACF free slot — both the `skipped_groups` side (a field group with an unsupported location rule) and the `dropped_fields` side (SEC-3 password drop) — via `tests/integration/Acf/AcfDiagnosticsLedgerTest`. Phase 1.1 also lands eight other known-regression integration tests (FIX-2 v0.6.1 empty url/email/date, Flex Content discriminator, label-only menu parent, zero-date draft, `wp_get_object_terms` grouping, posts with zero terms, FIX-5 v0.6.3 typed-block `anyOf`). The full Phase 1.1 inventory is in [`tests/README.md`](tests/README.md).
