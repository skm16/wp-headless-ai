...
import { prompt, promptPassword } from "../util/prompt.js";
 ...
 */
async function ensureValue(value: string | undefined, label: string): Promise<string> {
...
/**
 * Resolve the WP Application Password from --password flag, WP_APP_PASSWORD
 * env var, or a masked stdin prompt — in that priority order.
 */
async function resolvePassword(passwordFlag: string | undefined): Promise<string> {
