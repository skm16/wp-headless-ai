...
| `packages/cli` (`@jab/wp-headless-cli` 0.1.0) | Consumes the v0.6.0 `/wp-json/jab/v1/manifest` endpoint. `scaffold` / `init` / `generate` / `sync` working end-to-end against the pilot. **Still needs `jab sync` regen after upgrading consuming projects to v0.7.0+** so the generated SDK picks up the REQUIRED `modified` + `modified_gmt` row fields. v0.7.1 added no new manifest fields. |
