...
| 2.1 main — Phase B prompt context for image-bearing ACF fields | **Shipped** | `87ce1fc` | `summarizeAcfFields` annotates image/file/gallery/url + post-relation arrays with binding paths and no-magic warning; `acfFlexPrompt` gets a per-sample post-relation warning section via `findPostRelationFieldsInSample`; system prompt names the FeaturedBeer anti-example. |
| 2.4 main — broaden theme stylesheet capture filter | **Shipped** | `f01914c` | Two-tier capture: Tier 1 keeps strict `/wp-content/themes/` behavior; Tier 2 fires only when Tier 1 is empty and accepts ShortPixel / Autoptimize / WP Rocket / Swift Perf / NitroPack / generic `/wp-content/cache/` patterns. |
...
| K (P1) — MediaImage three-stage fallback for WP sourced-attr `core/image` (closes Fix-I regression) | **Shipped** | `1fdf555` | Fix I unconditionally routed `core/image` through MediaImage, but the shim only read `attrs.url ?? attrs.src`. The WP plugin's `BlockTypeSchema.php` documents that sourced attributes live in `innerHTML`, not stored `attrs` — typical `core/image` arrives with `attrs = {id, sizeSlug}` and the `<img>` markup in `innerHTML`. Pre-fix, the shim hit `if (!src) return null` and rendered nothing. Three-stage chain: (1) `attrs.url ?? attrs.src`, (2) `parseImgFromInnerHTML` extracts src/alt/width/height from the first `<img>` in `innerHTML` (attrs win on each field when both are present), (3) raw `innerHTML` passthrough for figures the parser can't decompose. `null` only when both are empty. SaaS shim + emitter mirror each other line-for-line; 4 new render-based stage-priority lock-ins (via `react-dom/server`) catch any future regression on the `!src` Stage-2 guard. |
