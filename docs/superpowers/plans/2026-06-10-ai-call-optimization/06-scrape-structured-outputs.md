...
6. **Nullable type representation = `["string","null"]` type arrays, not `anyOf`** (recorded post-review of Task 1). Phase 5's EDIT_PLAN_TOOL_SCHEMA used `anyOf` based on a docs reading that didn't list type arrays for tools strict mode. Current Anthropic structured-outputs docs (2026-06-10) confirm type arrays ARE supported for `output_config.format` and count against the same union-complexity budget as `anyOf`; `["string","null"]` is the terser, preferred form there. Both schemas are valid for their respective surfaces — do not "harmonize" one to the other without re-checking the docs for that surface.
